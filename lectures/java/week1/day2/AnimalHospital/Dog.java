...
public class Dog {
    ...
    // using a private access modifier meaning it can't be used outside of this
    // class we will need getters and setters to use them
    private String name;
    private String breed;
    private boolean goodDog = true;
    private int age;
    // Homework Challenge: instead of inputting the age of the dog, input the
    // birthdate and then have age be calculated on subtracting the birthdate from
    // today's date
    ...
    public Dog(String name, String breed, int age, boolean houseBroken) {
        ...
        Dog.dogCount += 1; // Using the static methods above to increase the dog count every time a new dog
                           // is created
        Dog.allDogs.add(this); // Also using the static method above to add the new dog to the list of all dogs
                               // created.
    }
    ...
    public String stats() {
        return String.format("Stats: %nName: %s%nAge: %s%nHousebroken: %s%nOwner: %s", this.name, this.age,
                this.houseBroken, this.owner.getOwnerName());
    }
